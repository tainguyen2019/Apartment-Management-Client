import { useEffect, useMemo, useState, useCallback } from 'react';
import { Grid, TableContainer, Paper } from '@material-ui/core';
import { useForm, DefaultValues, SubmitHandler } from 'react-hook-form';

import MyTable from 'components/common/my-table';
import SearchForm from 'components/common/search-form';
import Spin from 'ui/spin';
import MyPagination from 'components/common/my-pagination';
import usePagination from 'hooks/usePagination';
import useActions from 'hooks/useActions';
import useDidUpdate from 'hooks/useDidUpdate';
import useShallowEqualSelector from 'hooks/useShallowEqualSelector';
import useToggle from 'hooks/useToggle';
import * as actionCreators from 'redux/payslip/actionCreators';
import { selectPayslipState } from 'selectors/payslip';
import { Payslip, PayslipSearchFormValues } from 'types/payslip';
import AppContent from 'components/app/app-content';
import { appPaths } from 'routes/paths';
import { PRIVILEGES } from 'constants/users';
import Restriction from 'components/common/restriction';

import PayslipDialogContext from './PayslipDialogContext';
import EditDialogForm from './components/edit-dialog-form';
import CreateButton from './components/create-button';
import PayslipSearchForm from './components/payslip-search-form';
import { createPayslipCols } from './utils';
import { useStyles } from './styles';

const defaultSearchValues: DefaultValues<PayslipSearchFormValues> = {};

const PayslipPage: React.FC = () => {
  const classes = useStyles();
  const [getPayslips] = useActions(actionCreators.getPayslips);
  const { page, pageSize, handleChangePagination } = usePagination();
  const [isOpenEditForm, toggleEditForm] = useToggle();
  const [selectedPayslip, setSelectedPayslip] = useState<Payslip>();
  const { loading, payslips, totalPages } =
    useShallowEqualSelector(selectPayslipState);
  const {
    control,
    handleSubmit: submitForm,
    reset,
  } = useForm<PayslipSearchFormValues>({
    defaultValues: defaultSearchValues,
  });

  const handleEdit = (Payslip: Payslip) => () => {
    setSelectedPayslip(Payslip);
    toggleEditForm();
  };

  const handleClose = () => {
    setSelectedPayslip(undefined);
    toggleEditForm();
  };

  const PayslipCols = useMemo(
    () => createPayslipCols(handleEdit, page, pageSize),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [page, pageSize],
  );

  const fetchPayslips = useCallback<SubmitHandler<PayslipSearchFormValues>>(
    (values) => {
      getPayslips({
        ...values,
        page,
        pageSize,
      });
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [page, pageSize],
  );

  const handleSubmit = useMemo(
    () =>
      submitForm((values) => {
        // Call fetch Payslips
        if (page === 1) {
          fetchPayslips(values);
        }
        // Reset page to 1 and called later in the useEffect 'page'
        else {
          handleChangePagination(1, pageSize);
        }
      }),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [fetchPayslips],
  );

  const handleReset = () => {
    reset(defaultSearchValues);
    setTimeout(handleSubmit);
  };

  // Call on did mount and update page
  useEffect(() => {
    submitForm(fetchPayslips)();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page]);

  // Call on update page size
  useDidUpdate(() => {
    // Call submit form to get Payslip in case page = 1
    if (page === 1) {
      submitForm(fetchPayslips)();
    }
    // Change page to 1 and it will be revoked in the above useEffect 'page'
    else {
      handleChangePagination(1, pageSize);
    }
  }, [pageSize]);

  return (
    <AppContent title={appPaths.payslip().title}>
      <Spin loading={loading}>
        <Grid container direction="column" spacing={2}>
          <Grid item>
            <SearchForm onSubmit={handleSubmit} onReset={handleReset}>
              <PayslipSearchForm control={control} />
            </SearchForm>
          </Grid>
          <Grid item>
            <Restriction privilege={PRIVILEGES.createPayslip.value}>
              <CreateButton onRefresh={handleSubmit} />
            </Restriction>
          </Grid>
          <Grid item>
            <PayslipDialogContext.Provider
              value={{ open: isOpenEditForm, payslip: selectedPayslip }}
            >
              <TableContainer component={Paper}>
                <MyTable data={payslips} columns={PayslipCols} />
              </TableContainer>
              <EditDialogForm onClose={handleClose} onRefresh={handleSubmit} />
            </PayslipDialogContext.Provider>
          </Grid>
          {payslips.length > 0 && (
            <Grid item>
              <MyPagination
                classes={{
                  ul: classes.ulPagination,
                }}
                variant="outlined"
                shape="rounded"
                count={totalPages}
                page={page}
                pageSize={pageSize}
                onChange={handleChangePagination}
              />
            </Grid>
          )}
        </Grid>
      </Spin>
    </AppContent>
  );
};

export default PayslipPage;
